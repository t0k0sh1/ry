...
// ===== PairNodeData (two-child) cycle tests =====

TEST(GcTest, PairNodeSimpleCycleCollected) {
    // A -> {B, C}, B -> A, C -> null; A and B form a cycle, C is a leaf.
    // All have no external refs (strong_count = 1 from cycle members only).
    __ry_gc_enable();
    __ry_gc_set_threshold(10000);

    void *hdrA = gcAllocObj(sizeof(PairNodeData));
    void *hdrB = gcAllocObj(sizeof(NodeData));
    void *hdrC = gcAllocObj(sizeof(NodeData));

    auto *dataA = static_cast<PairNodeData *>(headerToData(hdrA));
    auto *dataB = static_cast<NodeData *>(headerToData(hdrB));
    auto *dataC = static_cast<NodeData *>(headerToData(hdrC));

    dataA->child_a = hdrB;
    dataA->child_b = hdrC;
    dataB->child_header = hdrA;
    dataC->child_header = nullptr;
    ...
    __ry_gc_track(hdrA, visitPairNode, dtorPairNode);
    __ry_gc_track(hdrB, visitNode, dtorNode);
    __ry_gc_track(hdrC, visitNode, dtorNode);
...
TEST(GcTest, PairNodeBothChildrenCycle) {
    // A -> {B, C}, B -> A, C -> A: A has two back-edges from B and C.
    __ry_gc_enable();
    __ry_gc_set_threshold(10000);
    ...
    dataA->child_b = hdrC;
    dataB->child_header = hdrA;
    dataC->child_header = hdrA;

    // A: 2 refs (from B and C)
    // B: 1 ref (from A)
    // C: 1 ref (from A)
    getHeader(hdrA)->strong_count = 2;
    getHeader(hdrB)->strong_count = 1;
...
TEST(GcTest, TwoIndependentCyclesCollectedInOnePass) {
    // Two separate cycles: A <-> B and C <-> D.
    // All objects should be collected in a single gc_collect() call.
    __ry_gc_enable();
    __ry_gc_set_threshold(10000);
    ...
    void *hdrB = gcAllocObj(sizeof(NodeData));
    void *hdrC = gcAllocObj(sizeof(NodeData));
    void *hdrD = gcAllocObj(sizeof(NodeData));

    static_cast<NodeData *>(headerToData(hdrA))->child_header = hdrB;
    static_cast<NodeData *>(headerToData(hdrB))->child_header = hdrA;
    static_cast<NodeData *>(headerToData(hdrC))->child_header = hdrD;
    static_cast<NodeData *>(headerToData(hdrD))->child_header = hdrC;

    getHeader(hdrA)->strong_count = 1;
    getHeader(hdrB)->strong_count = 1;
    getHeader(hdrC)->strong_count = 1;
    getHeader(hdrD)->strong_count = 1;

    __ry_gc_track(hdrA, visitNode, dtorNode);
    __ry_gc_track(hdrB, visitNode, dtorNode);
    __ry_gc_track(hdrC, visitNode, dtorNode);
    __ry_gc_track(hdrD, visitNode, dtorNode);

    int64_t collected = __ry_gc_collect();
    EXPECT_EQ(collected, 4);
}

TEST(GcTest, CollectCalledTwiceSecondCallReturnsZero) {
    // After a cycle is collected, a second collect() call should return 0.
    __ry_gc_enable();
    __ry_gc_set_threshold(10000);
