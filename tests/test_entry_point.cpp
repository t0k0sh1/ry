...
TEST_F(EntryPointTest, RunScriptFromPackageToml) {
    // `ry run <script-name>` executes the corresponding script command defined
    // in the [scripts] section of package.toml (added in this PR).
    // Scripts are executed via std::system(), so any shell command works.
    writeFile("package.toml",
              "[project]\nname = \"test\"\nversion = \"0.1.0\"\n"
              "entry = \"src/main.ry\"\n\n"
              "[scripts]\n"
              "greet = \"echo hello-from-script\"\n");
    writeFile("src/main.ry", "print(\"main\")");
    auto [out, rc] = runRyInDir(tmp_dir_.string(), {"run", "greet"});
    EXPECT_EQ(rc, 0);
    EXPECT_NE(out.find("hello-from-script"), std::string::npos);
}

TEST_F(EntryPointTest, RunScriptMissingNameShowsError) {
    // `ry run` with an unknown script name should exit non-zero and provide
    ...
    writeFile("package.toml",
              ...
              "build = \"src/build.ry\"\n");
    writeFile("src/main.ry", "print(\"main\")");
    auto [out, rc] = runRyInDir(tmp_dir_.string(), {"run", "nonexistent"});
    EXPECT_NE(rc, 0);
    // The output should mention the unknown script name.
    EXPECT_NE(out.find("nonexistent"), std::string::npos);
}

TEST_F(EntryPointTest, CStdinFlagExecutesStdin) {
    // With the -c flag, stdin code should be executed even without package.toml.
    auto [out, rc] = runRyInDir(tmp_dir_.string(), {"-c"}, "print(\"stdin-exec\")");
    EXPECT_EQ(rc, 0);
    EXPECT_EQ(out, "stdin-exec\n");
}