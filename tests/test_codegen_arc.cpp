...
// ===== Copy-on-Write (CoW) semantics tests =====
// When a collection is shared (multiple owners), mutation must produce an
// independent copy so the original remains unchanged.

TEST_F(CodeGenTest, CowListMutationDoesNotAffectOriginal) {
    // x and y initially share the same backing list.
    // Appending to y must not change x (CoW deep copy before mutation).
    EXPECT_EQ(runSource(
        "x = [1, 2, 3]\n"
        "y = x\n"
        "y = append(y, 4)\n"
        "print(length(x))\n"   // x is unchanged
        "print(length(y))\n"   // y has the new element
    ), "3\n4\n");
}

TEST_F(CodeGenTest, CowMapMutationDoesNotAffectOriginal) {
    EXPECT_EQ(runSource(
        "m1 = {\"a\": 1}\n"
        "m2 = m1\n"
        "m2[\"b\"] = 2\n"
...
TEST_F(CodeGenTest, CowListIndependentAfterCopy) {
    // Two independent copies of a list must have independent storage.
    EXPECT_EQ(runSource(
        "a = [10, 20]\n"
        "b = a\n"
        "b = append(b, 30)\n"
        "a = append(a, 99)\n"
        "print(a[2])\n"   // a[2] should be 99, not 30
        "print(b[2])\n"   // b[2] should be 30
    ), "99\n30\n");
}

// ===== Weak reference (codegen functional) =====

TEST_F(CodeGenTest, OptionNoneIsNone) {
    // Regression: make sure Option<int>::None doesn't cause ARC issues.
    EXPECT_EQ(runSource(
        "x: Option<int> = None\n"
        "when x:\n"
        "    case Some(v):\n"
...
TEST_F(CodeGenTest, OptionSomeRetainsValue) {
    EXPECT_EQ(runSource(
