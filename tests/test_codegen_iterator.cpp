...
TEST_F(CodeGenTest, IteratorLazyMap) {
    std::string src =
        "xs = [1, 2, 3]\n"
        "ys = xs.iter().map(function(x: int) => x * 10).to_list()\n"
        "print(ys[0])\n"
        "print(ys[1])\n"
        "print(ys[2])";
...
TEST_F(CodeGenTest, IteratorLazyMapUntypedLambdaParam) {
    std::string src =
        "xs = [1, 2, 3]\n"
        "ys = xs.iter().map(function(x) -> int => 2).to_list()\n"
        "print(ys[0])\n"
        "print(ys[1])\n"
        "print(ys[2])";
...
TEST_F(CodeGenTest, IteratorChained) {
    std::string src =
        "xs = [1, 2, 3, 4, 5]\n"
        "ys = xs.iter().filter(function(x: int) => x > 2).map(function(x: int) => x * 2).take(2).to_list()\n"
        "print(length(ys))\n"
        "print(ys[0])\n"
        "print(ys[1])";
    EXPECT_EQ(runSource(src), "2\n6\n8\n");
...
TEST_F(CodeGenTest, IteratorForLoopWithFilter) {
    std::string src =
        "xs = [1, 2, 3, 4, 5]\n"
        "sum = 0\n"
        "for x in xs.iter().filter(function(x: int) => x > 3):\n"
        "    sum = sum + x\n"
        "print(sum)";
