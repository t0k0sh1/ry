...
// Trace output is valid JSON Lines — every line must be parseable as a JSON object.
TEST_F(TraceModeTest, TraceOutputIsValidJsonLines) {
    auto script = writeFile("jsonl_check.ry", "print(42)\n");
    auto result = runRy({"--trace", script.string()});
    EXPECT_EQ(result.exit_code, 0);

    // Split stderr into lines and check each non-empty line starts with '{' and ends with '}'.
    std::istringstream ss(result.err);
    std::string line;
    int lineCount = 0;
    while (std::getline(ss, line)) {
        if (line.empty()) continue;
        ++lineCount;
        EXPECT_EQ(line.front(), '{') << "Line is not a JSON object: " << line;
        EXPECT_EQ(line.back(), '}') << "Line is not a JSON object: " << line;
    }
    EXPECT_GT(lineCount, 0) << "Expected at least one trace event line";
}

// Every trace event must carry an "event", "phase", and "ts" field.
TEST_F(TraceModeTest, TraceEventsHaveRequiredFields) {
    auto script = writeFile("fields_check.ry", "print(1)\n");
    auto result = runRy({"--trace", script.string()});
    EXPECT_EQ(result.exit_code, 0);
    ...
    while (std::getline(ss, line)) {
        if (line.empty()) continue;
        EXPECT_NE(line.find("\"event\""), std::string::npos) << "Missing 'event': " << line;
        EXPECT_NE(line.find("\"phase\""), std::string::npos) << "Missing 'phase': " << line;
        EXPECT_NE(line.find("\"ts\""), std::string::npos) << "Missing 'ts': " << line;
    }
}

// Trace with --trace-out must not create an empty file when tracing is active.
TEST_F(TraceModeTest, TraceOutFileIsNonEmpty) {
    auto script = writeFile("non_empty.ry", "print(\"x\")\n");
    fs::path tracePath = tmp_dir_ / "nonempty.jsonl";

    auto result = runRy({"--trace", "--trace-out=" + tracePath.string(), script.string()});
    EXPECT_EQ(result.exit_code, 0);
    EXPECT_GT(fs::file_size(tracePath), 0u);
}

// Without --trace, stderr should be empty (no spurious trace output).
TEST_F(TraceModeTest, NoTraceNoStderr) {
    auto script = writeFile("no_trace.ry", "print(\"clean\")\n");
    auto result = runRy({script.string()});
    EXPECT_EQ(result.exit_code, 0);
    EXPECT_EQ(result.out, "clean\n");
    EXPECT_TRUE(result.err.empty()) << "Unexpected stderr: " << result.err;
}

// Trace should capture the 'when' branch-selection event for pattern matching.
TEST_F(TraceModeTest, TraceCapturesWhenBranch) {
    auto script = writeFile(
        "when_branch.ry",
        "x: Option<int> = Some(5)\n"
        "when x:\n"
    ...
    // The 'branch.when' event should appear in the trace.
    EXPECT_NE(result.err.find("\"event\":\"branch.when\""), std::string::npos);
}