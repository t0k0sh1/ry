...
TEST_F(TraceModeTest, TraceEmitsBranchWhenEvent) {
    // Verify that the new `when` conditional syntax emits branch.when trace events.
    auto script = writeFile(
        "trace_when.ry",
    ...
    EXPECT_EQ(result.exit_code, 0);
    EXPECT_EQ(result.out, "1\n-1\n0\n");
    // The when statement should emit at least one branch event in the trace.
    bool has_branch =
        result.err.find("\"event\":\"branch.when\"") != std::string::npos ||
        result.err.find("\"event\":\"branch.if\"") != std::string::npos;
    EXPECT_TRUE(has_branch)
        << "Expected a branch trace event; got trace:\n" << result.err;
}

TEST_F(TraceModeTest, TraceOutputIsValidJsonLines) {
    // Every line of trace output must be a valid JSON object (starts with '{',
    // ends with '}') and must contain the "event" key.
    auto script = writeFile("jsonl_check.ry", "print(42)\n");
    auto result = runRy({"--trace", script.string()});
    EXPECT_EQ(result.exit_code, 0);

    std::istringstream stream(result.err);
    std::string line;
    int lines_checked = 0;
    while (std::getline(stream, line)) {
        if (line.empty()) continue;
        EXPECT_EQ(line.front(), '{')
            << "Trace line does not start with '{': " << line;
        EXPECT_EQ(line.back(), '}')
            << "Trace line does not end with '}': " << line;
        EXPECT_NE(line.find("\"event\""), std::string::npos)
            << "Trace line missing \"event\" key: " << line;
        ++lines_checked;
    }
    EXPECT_GT(lines_checked, 0) << "No trace lines found in stderr";
}

TEST_F(TraceModeTest, TraceOutFileCreatedAndNotEmpty) {
    auto script = writeFile("trace_size.ry", "print(\"done\")\n");
    fs::path tracePath = tmp_dir_ / "size_check.jsonl";

    auto result = runRy({"--trace", "--trace-out=" + tracePath.string(),
                         script.string()});
    EXPECT_EQ(result.exit_code, 0);
    EXPECT_TRUE(result.err.empty()) << "Expected no stderr output when --trace-out is set";
    EXPECT_TRUE(fs::exists(tracePath)) << "Trace file was not created";
    EXPECT_GT(fs::file_size(tracePath), 0u) << "Trace file is empty";
}