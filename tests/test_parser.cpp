...
// #2312 — after revert, `try = 1` must parse as an ordinary AssignStmt whose
// LHS identifier is "try" (the keyword no longer exists).
TEST(ParserTest, TryParsesAsIdentAfterRevert) {
    Program prog = parseStr("try = 1");
    ASSERT_EQ(prog.size(), 1u);
    ASSERT_TRUE(std::holds_alternative<AssignStmt>(prog[0]));
    const auto &assign = std::get<AssignStmt>(prog[0]);
    EXPECT_EQ(assign.name, "try");
    ASSERT_TRUE(std::holds_alternative<NumberExpr>(assign.value->data));
    EXPECT_EQ(std::get<NumberExpr>(assign.value->data).value, 1);
}
