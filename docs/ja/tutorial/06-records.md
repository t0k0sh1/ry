...
`match` と `case` を使って関連データを取り出します。バインディングにはフィールド名ではなく任意の変数名を使います。これは[制御構文](04-control-flow.md)で学んだパターンマッチングと直接つながります。
...
> **なぜ演算子オーバーロードなのか?** ドメイン型に自然な構文を与えます。`Vec2 + Vec2` は `vec2_add(a, b)` より読みやすく、`==` があれば `when` や比較でシームレスに動作します。

---

## 演習

1. **ADT**: `Dog(name: str)`、`Cat(name: str, indoor: bool)`、`Fish` のバリアントを持つ `Animal` enum を定義してください。`match` を使って各バリアントの説明を返す `describe(a: Animal) -> str` 関数を書いてください。
