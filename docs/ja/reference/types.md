...
- **暗黙の型変換はない** — `int` と `float` を混在させると float への昇格が発生するが、それ以外の暗黙変換は存在しない。`u8` は低レベル型であり、native 幅の unsigned 演算を行う。`u8` と `int` の混合演算はコンパイルエラーとなる。型アノテーション `b: u8 = 42` でのみ `int` リテラルから `u8` への縮小変換が許可される。
