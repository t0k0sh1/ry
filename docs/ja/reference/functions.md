...
- 関数は**トップレベル**で定義されている必要があります（別の関数内にネストされていないこと）。
...
ラムダ関数は定義された時点の外側スコープの変数を**値でキャプチャ**します。つまり、クロージャは独自の独立したコピーで動作します。外側からクロージャ方向、クロージャから外側方向のいずれの変更も、もう一方には影響しません。

### 外側の変更はクロージャに影響しない

```python
base = 10
add_base = (x: int) => x + base   # base を値でキャプチャ
...
| クロージャ内の変更の影響 | ない（外側スコープには伝播しない） |

> **Python/JavaScript ユーザーへの注意**: JavaScript ではクロージャは変数を参照でキャプチャするため、キャプチャされた変数への変更はクロージャの外側にも反映されます。Python ではクロージャは外側の変数にアクセスでき、キャプチャされたオブジェクトへの変更（例: リストへの追加）は外側からも見えますが、外側の名前の再バインド（例: `counter += x`）には `nonlocal` 宣言が必要です。Ry では、クロージャは常に値でキャプチャします。これは意図的なもので、特に並行処理や高階関数のコンテキストにおいて安全性と予測可能性を確保します。
