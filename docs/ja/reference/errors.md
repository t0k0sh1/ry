...
| match の網羅性不足 | `match` がすべてのパターンを網羅していない | enum の一部バリアントが未カバー、Option で `None` が欠落、Result で `Ok`/`Err` が欠落、リテラルに `_` なし |
| `?` の非 Result 型への適用 | `Result` 型でない式に `?` を適用した | `x = 42` → `x?` |
| `?` の非 Result 返却関数での使用 | `Result` を返さない関数内で `?` を使用した | `function foo() -> int:` → `bar()?` |
