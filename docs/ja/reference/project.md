...
ファイル引数が指定されず、標準入力がターミナルでない場合、`ry` は標準入力からソースコードを読み取って実行します:
...
1. **真正性**: `checksums.txt.sig` ファイルを組み込みの Ed25519 公開鍵で検証します。署名が存在しないか無効な場合、アップデートは中止されます。
2. **完全性**: アーカイブの SHA-256 ハッシュを `checksums.txt` と照合します。

署名検証をスキップするには（非推奨）、`RY_SKIP_SIGNATURE=1` を設定してください。
