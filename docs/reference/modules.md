...
| Unimported stdlib use | `<mod>.fn(...)` or `<mod>.field` where `<mod>` names a registered stdlib module (`math`, `json`, `path`, …) and `<mod>` itself is not in scope as an imported module is rejected at codegen with `module '<mod>' is not imported (add 'import <mod>' at the top of the file)`. Note that `import <mod> as <alias>` brings only `<alias>` into scope (see Alias above) — after `import math as m`, only `m.fn(...)` works; `math.fn(...)` is still rejected, and the user must reference the module via `<alias>`. A local variable that happens to share a stdlib module name shadows the package and is exempt from this check. (#1746) |
