...
| 9 | Implicit `any` → concrete unwrap | Variable decl: `src/codegen_stmt.cpp` Pattern 4 lint + `unwrapFromAny`. Function-call arg: `src/codegen_call_user.cpp:192-194`. Lambda-call arg: `src/codegen_call_dispatch.cpp` `coerceCallArgs`. `Ok(any)`/`Err(any)` into typed `Result` slots: `src/codegen_call.cpp`. | Variable decl warns (Pattern 4). All three remaining sub-cases (named-call arg, lambda-call arg, `Ok/Err` slot) are **silent today** and rely on a panicking runtime tag check in `unwrapFromAny`. | **deprecate** | #2321 | Variable decl: `tests/test_any_usage_warnings.cpp` `Pattern4*`. Named-call arg: `tests/spec/any.test.ry:207-232` "any unwrap in function call". Lambda-call arg: `tests/spec/any.test.ry` "any unwrap in lambda call" (added by #2320). `Ok/Err` slot: `tests/spec/any.test.ry` "any unwrap into Result slot" (added by #2320). |
...
### Silent sub-cases on Path 9

The new spec blocks added with this inventory pin only the **runtime-correctness half** of Path 9's three silent sub-cases (the unwrap succeeds and the unwrapped value matches expectations); they do not assert absence of diagnostics, since the goal of #2321 is precisely to add diagnostics at those sites without changing the runtime semantics. A regression that broke the runtime value or success outcome would still be caught.
