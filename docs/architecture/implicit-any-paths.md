...
| 9 | Implicit `any` → concrete unwrap | All sub-cases route through `CodeGen::emitImplicitUnwrapDiag` (`[[noreturn]]`). 9a variable decl: `src/codegen_stmt.cpp` `emitVarDecl`. 9b named fn-call arg (incl. default-value path): `src/codegen_call_user.cpp:192-204`, `:231-242`. 9c lambda-call arg: `src/codegen_call_dispatch.cpp` `coerceCallArgs`. 9d `Ok(any)`/`Err(any)`/`Some(any)` into typed `Result`/`Option` slots: `src/codegen_call.cpp`. 9e reassign of typed variable (local + module-global write-through, including the Result-slot widening pre-check before `coerceResultType`): `src/codegen_stmt.cpp` `emitStmt(AssignStmt&)` and `emitModuleGlobalWriteThrough`. 9f return from typed fn / lambda: `src/codegen_fn.cpp` `emitStmt(ReturnStmt&)` and `src/codegen_lambda.cpp` expr-body return coercion. 9g collection mutation: map `m[k] = v` and list `xs[i] = v` in `src/codegen_stmt_misc.cpp` `IndexAssignStmt`; `append!` / `appended` / `insert` / set `add` / set `remove` in `src/codegen_call_collection.cpp`. | All sub-cases reject as `[strict-any/any-implicit-unwrap]` hard errors via `CodeGen::strictAnyError`. `coerceResultType` per-slot Result→Result widening is rejected on reassignment (Path 9e) via a pre-check at the caller — the helper itself is unchanged so that variable declaration (`r: Result<int, str> = produce()`, Path 9a-style), the mock-return test infrastructure in `src/codegen_test.cpp`, and other internal callers continue to function. The runtime tag check inside `unwrapFromAny` is still reached by those non-rejecting callers, plus the `in` / `not in` membership query and the `get(list, idx, default)` fallback value (read-only operator surfaces — not slot-bound assignments the user introduced). | **rejected** | #2321 (warn + strict reject) → #2322 (default flip, opt-in removed) → #2379 (rule extended to reassign / return / collection mutation) | `tests/test_strict_any.cpp::AnyImplicitUnwrap*RejectedByDefault` pins each sub-case. |
