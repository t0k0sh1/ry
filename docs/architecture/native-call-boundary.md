...
- **Primary: `base64`.** Eight A1 entries (`src/codegen_call_base64.cpp:9-31`) chosen because they exercise three descriptor fields at once — `return_wrapping` (`decode` returns `Result<...>`), `error_channel` (`__ry_base64_get_last_error`), `require_list_u8_arg` (`encodeBytes` / `encodeBytesUrlSafe` gate the arg-0 byte list). `resource_kind` is intentionally absent: base64 is pure data transformation, which keeps the descriptor surface minimal while still proving the three fields simpler A1 modules omit.
- **Confirmation: `path`.** Six A1 entries (`src/codegen_call_io.cpp:1215`). No error channel, no Result wrapping, no resource — confirms the descriptor's `library_name` path and the no-wrapping case.
