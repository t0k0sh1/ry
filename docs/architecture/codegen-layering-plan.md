...
| **2-C** | Successively migrate the remaining lowered IR ops (`RuntimeCall`, `AnyWrap` / `Unwrap`, `CollectionMutate`, `CowEnsureUnique`, `ControlFlow`). Start crossing category 1 (LLVM context handles) and category 2 (primitive type accessors) so the transitional `void*` parameters can become typed opaque handles. **Landed**: `OptionWrap` via #1967; `ArcRetain` / `ArcRelease` via #1968 (`ry_emit_arc_retain` / `ry_emit_arc_release` ABI entry points, `lowered::ArcRetainOp` / `ArcReleaseOp`, atomic flag and destructor `void*` cross the ABI); `RuntimeCall` via #1969 (`ry_emit_runtime_call` ABI entry point, `lowered::RuntimeCallOp`, ~130 scattered `getOrInsertFunction("__ry_…") + CreateCall` pairs consolidated to `getRuntimeFn(...)` across 17 codegen TUs, two variadic sites in `codegen_call_user.cpp` deferred); `CowEnsureUnique` via #1970 (`ry_emit_cow_ensure_unique` ABI entry point with `RyCowEnsureUniqueDesc` carrying kind + atomic flag + per-kind element/key sizes + ARC retain flags + destructor `void*`, internal use of `ry_emit_arc_retain` / `ry_emit_arc_release` for element retain loops and old-data release, anonymous List/Map/Set header structs rebuilt locally inside the ABI); `CollectionMutate` via #1971 (`ry_emit_collection_append` / `_insert` / `_remove_at` / `_list_slice` ABI entry points, `lowered::CollectionAppendOp` / `CollectionInsertOp` / `CollectionRemoveAtOp` / `ListSliceOp`, header/elem type pointers and `elem_size` cross the ABI; slice uses `RyValueId *out_count, RyValueId *out_new_data` out-parameters to avoid struct-by-value across the ABI); `AnyWrap` / `AnyUnwrap` / `AnyTryUnwrap` via #1972 (`ry_emit_any_wrap` / `ry_emit_any_unwrap` / `ry_emit_any_try_unwrap` ABI entry points, `lowered::AnyWrapOp` / `AnyUnwrapOp` / `AnyTryUnwrapOp`, each ABI helper carries its own `RyAnyWrapKind` / `RyAnyUnwrapKind` / `RyAnyTryUnwrapKind` per-kind dispatch; sub-emission helpers `tryUnwrapRecordFromAny` / `tryUnwrapListFromAny` / `tryUnwrapMapFromAny` / `tryUnwrapOptionFromAny` / `unwrapEnumFromAny` stay CodeGen-private per Path 1 narrower design; type-name resolution, descriptor lookup, layout-type construction, tag computation, typed-collection registration via `__ry_any_register_typed_coll`, field-wise ARC retain via `emitRecordArcFieldsRetain` / `emitEnumBoxArcFieldsRetain`, and `propagateTypeMeta` re-stamping stay on the CodeGen-side shim). Remaining Stage 2-C op (`ControlFlow`) still pending. | The codegen call sites no longer call `IRBuilder<>` directly; the LLVM-owning side is reachable only through the ABI. | #1965 (Stage 2-C umbrella); #1967 (OptionWrap); #1968 (ARC); #1969 (RuntimeCall); #1970 (CowEnsureUnique); #1971 (CollectionMutate); #1972 (AnyWrap/Unwrap). |
