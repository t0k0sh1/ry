...
| **3** | Reimplement the shared library in Rust behind the same ABI. | All existing tests pass with the Rust implementation; the C++ implementation is removed (or feature-flagged for the brief transition). | #1950 (scaffolding); #1993 (cutover, landed). |
...
1. Step 2 (#1949) has landed — the shared library is built and the codegen call sites dispatch through the ABI. Step 3 (#1950 scaffolding, #1993 cutover) has also landed — the shared library is now implemented in Rust (`crates/ry_llvm_emit/`) and the C++ implementation is removed.
