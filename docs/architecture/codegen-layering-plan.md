...
| **2-B** | Migrate the remaining category-3 helpers (`wrapPtrAsResult`, `wrapStatusAsResult`, `emitResultBranch`) together with the `ResultWrap` lowered op. Replace `emitNegativeIndexWrap` / `emitBoundsError` callbacks with proper ABI functions. | All category-3 helpers cross the ABI; BoundsCheck has no remaining `CodeGen` callbacks. | Follow-up issue (TBD). |
