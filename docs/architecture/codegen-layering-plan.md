...
1. Step 2 (#1949) has landed — the shared library is built and the codegen call sites dispatch through the boundary. Step 3 (#1950 scaffolding, #1993 cutover) has also landed — the shared library is now implemented in Rust (`crates/ry_llvm_emit/`) and the C++ implementation is removed.
