...
| **2-C** | Successively migrate the remaining lowered IR ops (`RuntimeCall`, `AnyWrap` / `Unwrap`, `CollectionMutate`, `ArcRetain` / `Release`, `CowEnsureUnique`, `ControlFlow`); `OptionWrap` landed first via #1967. Start crossing category 1 (LLVM context handles) and category 2 (primitive type accessors) so the transitional `void*` parameters can become typed opaque handles. | The codegen call sites no longer call `IRBuilder<>` directly; the LLVM-owning side is reachable only through the ABI. | #1965 (Stage 2-C umbrella); #1967 (OptionWrap). |
