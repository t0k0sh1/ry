...
索引和字段賦值可以組合：`=` / `+=` / `-=` / `*=` / `/=` / `//=` / `%=` / `**=` / `&=` / `|=` / `^=` / `<<=` / `>>=` 的左側可以是任何根植於可變變量的後綴鏈。
...
複合賦值對每個索引恰好求值一次，因此 `xs[f()] += 1` 只調用 `f()` 一次。對缺失映射鍵的複合賦值是運行時錯誤 — 如果想要累加，請先插入鍵：

```python
m = {"a": 1}
m["a"] += 10            # OK  → {"a": 11}
m["b"] += 10            # 運行時錯誤：對缺失映射鍵的複合賦值
```

> **嵌套寫入是隔離的**：像 `grid[i][j] = v` 和 `r.items[i] = v`（通過包含列表的記錄字段）這樣的鏈式寫入會在變更發生之前私有化 LHS 路徑上引用計數 > 1 的每一層，因此外部容器的別名 — 或路徑上任何內部容器 — 會觀察到寫入前的狀態。詳細信息請參閱下面的 [Path Copy-on-Write](#path-copy-on-write)。
...
某些列表操作有兩種形式：返回新列表的非變更版本，以及名稱以 `!` 結尾的就地變體。當您打算變更接收器並希望在調用點明確此意圖時，請使用 `!` 形式；當您希望保留原始值時，請使用非變更形式。

| 就地（`!`） | 非變更等價 | 區別 |
|------|-----|-----|
| `append!(xs, v)` / `append(xs, v)` | `appended(xs, v)` | `append!` / `append` 就地變更並返回 `Unit`；`appended` 返回新列表 |
| `sort!(xs)` / `sort!(xs, cmp)` | `sort(xs)` / `sort(xs, cmp)` | `sort!` 就地變更 `xs`；`sort` 返回新的排序列表 |
| `reverse!(xs)` | `reverse(xs)` | `reverse!` 就地變更 `xs`；`reverse` 返回新的反轉列表 |

所有 `!` 變體都參與 Copy-on-Write：如果 `xs` 是共享的（引用計數 > 1），外部緩衝區會在就地變更前克隆一次，以使別名不受影響（請參閱 [Copy-on-Write (CoW) 語意](#copy-on-write-cow-语义)）。
...
- 當讀者受益於看到原始值被保留時（例如函數式管道，或當後續仍然需要原始值時），優先使用 `sort`、`reverse`、`appended`。
- 當原始值真的被替換時，優先使用 `sort!`、`reverse!`、`append!`，以避免分配中間副本，並在調用點明確變更意圖。
...
- **變更觸發路徑遍歷複製**：當共享集合被變更時，LHS 路徑上引用計數 > 1 的每一層都會在變更發生之前被克隆。頂層寫入（`b.append(...)`、`b[i] = v`，當 `b` 是共享時）僅克隆最外層容器。鏈式寫入（`b[i][j] = v`、`r.items[i] = v`、`m[k1][k2] = v`）從根遍歷到底，並克隆仍然共享的每個中間容器，因此外部容器的別名 — 或路徑上的任何內部容器 — 都會與變更隔離。
...
Path CoW 處理通過嵌套集合的鏈式寫入 — 寫入器從 LHS 根變量（或記錄字段）遍歷到葉寫入點，並私有化引用計數大於一的每一層。這在共享外部容器和路徑上任何內部容器的別名之間提供嚴格的別名隔離。

```python
a = [[1, 2], [3, 4]]
b = a                    # 外部列表共享：strong_count = 2
b[0][0] = 99             # 遍歷：克隆外部（a 和 b 現在有自己的
...
帶有 ARC 管理的集合值的記錄字段以與直接索引跳轉相同的方式參與 path CoW。記錄到記錄的賦值（`r2 = r1`）保留每個 ARC 字段，因此後續的 `r2.items[i] = v` 會在字段槽觀察到 refcount > 1 並在變更前克隆：
...
**不支持**作為 path-CoW 鏈的根：方法調用 lvalue（`f().x[i] = v`）和在記錄字段遍歷內穿插索引跳轉的鏈（`rec.arr[0].items[i] = v`）。這些形狀會產生編譯時錯誤；請先將中間值賦給局部變量。
