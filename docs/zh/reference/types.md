...
| `u8` | i8 | （無專用字面值） | 無號 8 位元整數（0-255）。透過型別標註 `b: u8 = 42` 使用 |
...
| `[T; N]` | `[N x T]` | `buf: [i32; 8]` | 固定長度陣列。低階型別 T 的 N 個元素（堆疊配置、連續記憶體） |
...
- **沒有隱式型別轉換** — 混合使用 `int` 和 `float` 時會發生 float 提升，但除此之外不存在隱式轉換。`u8` 是低階型別，使用原生寬度的無號運算；`u8` 與 `int` 的混合運算會產生編譯錯誤。僅在型別標註 `b: u8 = 42` 時允許從 `int` 字面值到 `u8` 的窄化轉換。
