...
llvm::Function *CodeGen::resolveOverload(const std::string &callee,
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        if (isNone[i]) {
            ...
            // Path 9b: f(anyVal) into concrete param. Rejected by
    ...
    for (size_t i = args.size(); i < chosen->paramTypes.size(); ++i) {
        ...
        if (defVal->getType() != chosen->paramTypes[i] &&
            ...
            // Path 9b default-value path: same rejection as the explicit-arg
            // branch above.
            const std::string pname =
                i < chosen->paramNames.size() && !chosen->paramNames[i].empty()
                    ? chosen->paramNames[i] : "arg " + std::to_string(i);
...
llvm::Value *CodeGen::emitCheckedFPToInt(llvm::Value *val, llvm::Type *targetTy,
    ...
    // Thin wrapper over the cdylib op (#2097). Compute the message string,
    // global-name hint (counter-stamped on the C++ side so the cdylib stays
    // stateless), bit width, and signedness; the engine emits the FPExt /
    // range check / failBB+okBB split / runtime-error exit / FPToSI|FPToUI.
    ...
    RyValueId id = ry_emit_checked_fp_to_int(emit_ctx_, valId,
                                              static_cast<int>(bits),
                                              isUnsigned ? 0 : 1,
                                              bbPrefix.c_str(), msg.c_str(),
                                              globalName.c_str());
