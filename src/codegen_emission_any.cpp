...
llvm::Value *emitAnyWrap(CodeGen &cg, const lowered::AnyWrapOp &op) {
    ...
    desc.val_id = ry_emit_intern(cg.emit_ctx_, ry::llvm_emit::asRyValue(op.val));
    desc.do_collection_retain = op.do_collection_retain ? 1 : 0;
    desc.do_str_retain = op.do_str_retain ? 1 : 0;
    desc.descriptor_id =
        op.descriptor ? ry_emit_intern(cg.emit_ctx_, ry::llvm_emit::asRyValue(op.descriptor)) : 0;
    desc.box_layout_ty = ry::llvm_emit::asRyType(op.box_layout_ty);
    desc.box_data_size = op.box_data_size;
    desc.any_ty = ry::llvm_emit::asRyType(op.any_ty);
...
llvm::Value *emitAnyUnwrap(CodeGen &cg, const lowered::AnyUnwrapOp &op) {
    ...
    desc.any_ty = ry::llvm_emit::asRyType(op.any_ty);
    desc.target_ty = ry::llvm_emit::asRyType(op.target_ty);
    ...
    desc.expected_desc_id =
        op.expected_desc ? ry_emit_intern(cg.emit_ctx_, ry::llvm_emit::asRyValue(op.expected_desc)) : 0;
    desc.box_layout_ty = ry::llvm_emit::asRyType(op.box_layout_ty);
    desc.record_struct_ty = ry::llvm_emit::asRyType(op.record_struct_ty);
...
llvm::Value *emitAnyTryUnwrap(CodeGen &cg, const lowered::AnyTryUnwrapOp &op) {
    ...
    desc.any_val_id = ry_emit_intern(cg.emit_ctx_, ry::llvm_emit::asRyValue(op.any_val));
    desc.any_ty = ry::llvm_emit::asRyType(op.any_ty);
    desc.res_ty = ry::llvm_emit::asRyType(op.res_ty);
    desc.error_ty = ry::llvm_emit::asRyType(op.error_ty);
    desc.target_ty = ry::llvm_emit::asRyType(op.target_ty);
    desc.expected_tag = op.expected_tag;
    desc.do_collection_retain = op.do_collection_retain ? 1 : 0;
    desc.do_str_retain = op.do_str_retain ? 1 : 0;
    desc.err_msg_str_id =
        op.err_msg_str ? ry_emit_intern(cg.emit_ctx_, ry::llvm_emit::asRyValue(op.err_msg_str)) : 0;
