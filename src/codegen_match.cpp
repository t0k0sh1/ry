            ...
            if (enumIt != enum_types_.end()) {
                ...
                if (fit != enumIt->second.variantFields.end()) {
                    ...
                    for (size_t bi = 0; bi < pat.bindings.size() && bi < fit->second.fieldTypes.size(); ++bi) {
                        ...
                        // ARC retain: use the field's source-level type name so the
                        // correct destructor is registered for collections/str.
                        const std::string &fieldSig =
                            (bi < fit->second.fieldTypeNames.size())
                                ? fit->second.fieldTypeNames[bi] : std::string{};
                        if (!fieldSig.empty())
                            propagateTypeMeta(fieldSig, bindAlloca);
                        emitPatternBindingArc(fieldVal, bindAlloca, fieldSig);
                        offset += dl.getTypeAllocSize(fieldTy);
                    }
                }
            }
