...
void emitCollectionAppend(CodeGen &cg,
    ...
    RyValueId listId = ry_emit_intern(cg.emit_ctx_, op.list_ptr);
    RyValueId valId = ry_emit_intern(cg.emit_ctx_, op.val);
...
llvm::Value *emitCollectionRemoveAt(CodeGen &cg,
    ...
    RyValueId removedId = ry_emit_collection_remove_at(
        ...
        static_cast<void *>(op.list_header_ty),
        static_cast<void *>(op.elem_ty), op.elem_size);
    return static_cast<llvm::Value *>(ry_emit_resolve(cg.emit_ctx_, removedId));
}
...
lowered::ListSliceResult emitListSlice(CodeGen &cg,
    ...
    ry_emit_ctx_set_function(cg.emit_ctx_, cg.fn_);
    RyValueId listId = ry_emit_intern(cg.emit_ctx_, op.list_ptr);
    RyValueId startId = ry_emit_intern(cg.emit_ctx_, op.start);
    RyValueId endId = ry_emit_intern(cg.emit_ctx_, op.end_excl);
    RyValueId countId = 0;
    ...
    result.count =
        static_cast<llvm::Value *>(ry_emit_resolve(cg.emit_ctx_, countId));
    result.new_data =
        static_cast<llvm::Value *>(ry_emit_resolve(cg.emit_ctx_, newDataId));
    return result;
}
