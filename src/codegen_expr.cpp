...
// ===== TryBlockExpr (#1702 effect-block) =====
//
// `try:` block-as-expression. The tail expression is implicitly wrapped in
// Ok(...) / Some(...); any `?` operator inside escapes to this block's merge
// (not the enclosing function). Context (Result vs Option, and the Err type)
// is resolved from the enclosing fn return type. Let-annotation context is
// added in #43 via try_block_ctx_.
//
// BB scaffold:
//   try.body  (emit body + tail here, falls through to try.ok.merge)
//   try.ok.merge  → try.merge
//   try.err.merge (each `?` site CreateBr here; addIncoming to err_phi)
//                 → try.merge
//   try.merge  PHI(ok_wrapped, err_wrapped)
llvm::Value *CodeGen::emitExprVariant(const std::unique_ptr<TryBlockExpr> &e) {
    // Context order:
    //   1. try_block_ctx_  — let-annotation hint (`let x: Result<T,E> = try:`)
    //   2. fn_->getReturnType()  — `return try: ...` in a typed fn
    // Without either, the type is unknowable; emit an actionable error
    // pointing the user at the two supported forms.
    //
    // Snapshot-and-clear `try_block_ctx_` so the hint never flows into nested
    // try: blocks (`let x: Result<int,E> = try: { let y = try: ... ; y }` must
    // not silently inherit the outer annotation for the inner). Mirrors the
    // `option_decl_annotation_inner_` discipline.
    llvm::Type *ctxTy = try_block_ctx_;
    try_block_ctx_ = nullptr;
    if (!ctxTy && fn_)
        ctxTy = fn_->getReturnType();
    bool is_option = ctxTy && isOptionType(ctxTy);
    bool is_result = ctxTy && isResultType(ctxTy);
    if (!is_option && !is_result)
        codegenError("'try:' block requires type context: annotate `let x: Result<T,E> = try:` "
                     "or use inside a fn returning Result/Option");

    auto *fullResultTy = llvm::cast<llvm::StructType>(ctxTy);
    llvm::Type *expectedErrTy = is_result ? fullResultTy->getElementType(2) : nullptr;

    llvm::BasicBlock *errMergeBB = createBB("try.err.merge");
    llvm::BasicBlock *mergeBB = createBB("try.merge");

    // Pre-create the PHI at err.merge so `?` sites can addIncoming. For Option,
    // no PHI is needed (None has no payload).
    llvm::PHINode *errPhi = nullptr;
    if (is_result) {
        llvm::BasicBlock *savedBB = builder_.GetInsertBlock();
        builder_.SetInsertPoint(errMergeBB);
        errPhi = builder_.CreatePHI(expectedErrTy, 0, "try.err.raw");
        builder_.SetInsertPoint(savedBB);
    }

    try_scope_stack_.push_back({errMergeBB, scope_stack_.size(), errPhi, is_option});

    // Emit body statements + tail. The tail expression is the block's value
    // before Ok/Some wrap.
    pushScope();
    auto &mutBody = const_cast<std::vector<StmtNode> &>(e->body);
    for (size_t i = 0; i < mutBody.size(); ++i)
        std::visit([this](auto &s) { emitStmt(s); }, mutBody[i]);
    ...
    llvm::BasicBlock *okPathEndBB = builder_.GetInsertBlock();
    builder_.CreateBr(mergeBB);

    // Fill err.merge: construct Err(errPhi, fullResultTy) or None, branch to merge.
    builder_.SetInsertPoint(errMergeBB);
    llvm::Value *errValWrapped;
    if (is_result)
        errValWrapped = buildErrValue(errPhi, fullResultTy);
    else
        errValWrapped = buildNoneValue(fullResultTy);
    builder_.CreateBr(mergeBB);

    builder_.SetInsertPoint(mergeBB);
    llvm::PHINode *phi = builder_.CreatePHI(fullResultTy, 2, "try.block");
    phi->addIncoming(okValWrapped, okPathEndBB);
    phi->addIncoming(errValWrapped, errMergeBB);
    propagateMeta(okValWrapped, phi);
    return phi;
}
...
llvm::Value *CodeGen::emitExprVariant(const std::unique_ptr<ErrorPropagateExpr> &e) {
    ...
    // #1702 — `try:` block escape (innermost enclosing try-scope wins). When the
    // try-scope-stack is empty, fall through to the existing fn-return-based
    // early-return path to preserve byte-identical behavior for existing tests.
    if (!try_scope_stack_.empty()) {
        TryScope &ts = try_scope_stack_.back();
        if (ts.is_option && operandIsResult)
            codegenError("'?' on Result inside an Option 'try:' block — mixing Result and Option propagation is not allowed");
        if (!ts.is_option && operandIsOption)
            codegenError("'?' on Option inside a Result 'try:' block — mixing Result and Option propagation is not allowed");
        ...
        if (operandIsResult) {
            ...
            llvm::Type *expectedErrTy = ts.err_phi->getType();
            if (errVal->getType() != expectedErrTy) {
                if (auto *sliced = tryEmitSubtypeCoerce(errVal, expectedErrTy))
                    errVal = sliced;
                else
                    codegenError("'?' err type does not match the enclosing 'try:' block's err type");
            }
            retainBlockTailIfScopeOwned(errVal);
            emitScopeCleanupToDepth(ts.scope_depth);
            ts.err_phi->addIncoming(errVal, builder_.GetInsertBlock());
            builder_.CreateBr(ts.err_merge_bb);
        ...
        // Option operand in an Option try: block.
