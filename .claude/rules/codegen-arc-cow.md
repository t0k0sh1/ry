...
### `tryRetainArcSource` LoadInst cases must be metadata-gated — and so must every `-24` retain dispatch that uses `isStringValue` as its selection predicate
...
**The same metadata-gate discipline applies to every other site that picks the StringHeader offset (−24) for retain or release**, not just `tryRetainArcSource`. `isStringValue(val)` (`src/codegen_any.cpp`) is a **negative-evidence predicate** (`ptrTy_ && !isNonStrPointer`) — true on any metadata-less `ptrTy_` value, which includes the container-element fresh load `load ptr, %elem_ptr` inside `filter` / `slice` / `map` bodies. Using `isStringValue` alone to choose `−24` over `−16` reproduces exactly the trap `#1266` already taught for `propagateTypeMeta("str", elem)`: a Map / List / Set header data pointer gets retained at `−24`, and the errant `+1` writes into the adjacent allocation's tail 8 bytes — visible to glibc as `invalid next->prev_inuse` (Linux SIGSEGV) and to macOS as a silent heap corruption that surfaces downstream (KNOWLEDGE.md "map key not found" / #2246). `#1799` introduced this same pattern into `wrapInAny`'s str retain branch and #2246 patched it by adding a positive-evidence gate (`arc_str_owned_values_` / `arc_str_managed_vars_` / `GlobalVariable` / `meta->str_elem`).

**How to apply**: any new `emitStrGetHeaderFromData(val)` / `emitArcRetain(strHdr)` / `emitArcRelease(strHdr)` site whose `val` could be a `ptrTy_` value loaded from a container slot MUST require positive str evidence before the dispatch. Acceptable evidence channels (mirror the `#2246` gate in `CodeGen::wrapInAny`):

- `arc_str_owned_values_` membership (str produced by `makeString` / `__ry_string_make_uninit` / `+` concat / runtime construction)
- `arc_str_managed_vars_` membership (a `LoadInst` whose pointer operand is a tracked str alloca)
- `llvm::isa<llvm::GlobalVariable>(val)` (literal global from `cachedGlobalString`)
- `getMeta(val)->str_elem == true` (container-element str stamped by the List/Map/Set indexer)

`isStringValue` alone is **never** sufficient. Tracking issue for the long-term fix (replace `isStringValue`'s body with a positive-evidence predicate and audit all 48 callsites): `#2248`. The upstream prevention — propagating container metadata onto higher-order combinator element loads so the metadata-gate is never tripped — is **landed in #2247**: snapshot the source container's `list_elem_type_name` (or `map_value_type_name` for the Map arm of `emitCollOp_get`) into a local string **before** each loop body and call `propagateTypeMeta(snap, elem)` immediately after every fresh `load ptr, %elem_ptr` — `filter` / `map` / `reduce` / `fold` / `any` / `all` / `tap` element loads plus the Map arm `get*_val`. With those stamps in place, `wrapInAny` / `__ry_any_eq` / `unwrapFromAny` route through the correct collection tag on the upstream side and the `#2246` retain gate is no longer the first line of defence. `emitMapMergeCore`'s `mg_vv2` is **deliberately out of scope** — `mgValIsArc` already excludes `CollectionKind::Str` (see #1266) and for non-str collection values the metadata-less fall-through `emitArcGetHeaderFromData(val)` correctly hits the ArcHeader at `-16`. The iterator path (`for elem in iter(xs):`) remains a known gap with a separate follow-up. Regression detection lives in `tests/test_regression_2246_str_metadata_gate.cpp` (subprocess loops) plus `tests/spec/higher_order_elem_metadata.test.ry` (`@it` positive-correctness + `__ry_any_eq` tag-mismatch detector); the bug is heap-layout-sensitive and does NOT reproduce inside an `@it` block (testing-framework scaffolding stabilises the heap), so subprocess loops are the only reliable detector short of sanitizer instrumentation.
