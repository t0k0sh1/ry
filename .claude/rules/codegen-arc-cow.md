...
### `tryRetainArcSource` LoadInst cases must be metadata-gated — and so must every `-24` retain dispatch; `isStringValue` is the canonical positive-evidence gate
...
**The same metadata-gate discipline applies to every other site that picks the StringHeader offset (−24) for retain or release**, not just `tryRetainArcSource`. The recurring trap (`#1266` for `propagateTypeMeta("str", elem)` and `#1799` for `wrapInAny`'s str retain branch) was: a Map / List / Set header data pointer gets retained at `−24`, and the errant `+1` writes into the adjacent allocation's tail 8 bytes — visible to glibc as `invalid next->prev_inuse` (Linux SIGSEGV) and to macOS as a silent heap corruption that surfaces downstream (KNOWLEDGE.md "map key not found" / #2246). `#2246` patched `wrapInAny` locally; `#2247` propagated container metadata onto higher-order combinator element loads so the gate is never tripped from those shapes; **`#2248` rewrote `CodeGen::isStringValue` itself as a positive-evidence predicate** so the structural property — "no `-24` dispatch from a metadata-less container-element fresh load" — is enforced at the predicate, not at every caller.

**Predicate selector** (`src/codegen_any.cpp` + `src/codegen_arc.cpp`):

| Predicate | Semantics | When to use |
|---|---|---|
| `isStringValue(val)` | **Positive-evidence**: true iff one of four channels confirms str — `arc_str_owned_values_`, `LoadInst` into `arc_str_managed_vars_`, `GlobalVariable` / GEP-to-`GlobalVariable` (immortal literal from `cachedGlobalString`), or `meta->str_elem`. False on every other `ptrTy_`. | Drives `-24` ARC retain / release dispatch and any other path that picks the StringHeader offset. New `emitStrGetHeaderFromData` callers must gate on this. |
| `isStrLike(val)` | **Negative-evidence** (legacy `isStringValue` body): `ptrTy_ && !isNonStrPointer`. True on every metadata-less `ptrTy_` — including the container-element fresh load that broke `#1266` / `#1799` / `#2246`. | Type-discrimination dispatch where a false positive yields a recoverable compile error, not heap corruption: `+` / `==` / `in` operator dispatch, argument validation (`open(path, mode)` / `replace(s, /re/, repl)` / `json.load[T]` / `fail(msg)`), branch-type classification (`codegen_match.cpp:83`), literal `*_type_name = "str"` stamping (the `anyXxxIsStrLike` flag), `for c in s:` / `enumerate(s)` / `zip(s, …)` str-fallback desugaring. Never call from a `-24` dispatch. |
| `isStrHandle(val)` | **Strict** (subset of `isStringValue`): `arc_str_owned_values_` ∪ load-from-`arc_str_managed_vars_`. **Excludes** immortal globals. | Retain loops where the immortal global is a no-op anyway and skipping it elides useless `arc_inc` calls (e.g. the per-entry retain in literal collection construction). |

**How to apply** — new `emitStrGetHeaderFromData(val)` / `emitArcRetain(strHdr)` / `emitArcRelease(strHdr)` sites: call `isStringValue(val)`. The four channels above live inside its body; no inline re-check is needed. New type-discrimination callers (dispatch, validation, diagnostic, metadata stamp): call `isStrLike(val)`. Per-entry retain loops over literal collections: call `isStrHandle(val)`.

The upstream prevention — propagating container metadata onto higher-order combinator element loads so the source-side trap shape is also gone — is **landed in #2247**: snapshot the source container's `list_elem_type_name` (or `map_value_type_name` for the Map arm of `emitCollOp_get`) into a local string **before** each loop body and call `propagateTypeMeta(snap, elem)` immediately after every fresh `load ptr, %elem_ptr` — `filter` / `map` / `reduce` / `fold` / `any` / `all` / `tap` element loads plus the Map arm `get*_val`. With those stamps in place, `wrapInAny` / `__ry_any_eq` / `unwrapFromAny` route through the correct collection tag on the upstream side. `emitMapMergeCore`'s `mg_vv2` is **deliberately out of scope** — `mgValIsArc` already excludes `CollectionKind::Str` (see #1266) and for non-str collection values the metadata-less fall-through `emitArcGetHeaderFromData(val)` correctly hits the ArcHeader at `-16`. The iterator path (`for elem in iter(xs):`) remains a known gap with a separate follow-up. Regression detection lives in `tests/test_regression_2246_str_metadata_gate.cpp` and `tests/test_regression_2248_positive_str_predicate.cpp` (subprocess 100-iter loops, the only reliable detector — the bug is heap-layout-sensitive and the testing-framework scaffolding inside an `@it` block stabilises the heap and masks the failure) plus `tests/spec/higher_order_elem_metadata.test.ry` (positive-correctness + `__ry_any_eq` tag-mismatch detector).
...
`str` is `ptrTy_` and passes the "must be ptr" gate. After the "index operator requires list or map" gate and BEFORE Map dispatch, check `isStrLike(objPtr)` and emit targeted diagnostics: read path → `"str does not support index access; use charAt(s, i) instead"`, write path → `"str does not support index assignment; strings are immutable"`. The write-path guard MUST come before `emitCowCheck` — passing a `str` pointer with `CollectionKind::List` is unsafe. Use the loose `isStrLike` (not the strict `isStringValue`) because the predicate's job here is "reject any `ptrTy_` that isn't a known collection" — including a `str` returned by `getName()` (a `CallInst` outside every positive-evidence channel). `isStringValue` would let that user-fn-returned str slip past the guard and into the `emitCowCheck` / `getListElementType` dispatch.
