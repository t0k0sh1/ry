...
- **Linux**: set the rpath in CMake, not patchelf. Use `BUILD_WITH_INSTALL_RPATH ON` with `INSTALL_RPATH`, not `BUILD_RPATH`. `BUILD_RPATH` is additive to CMake's automatic link-path rpath, so the shipped binary keeps build-machine-absolute paths (`/usr/local/llvm/lib`, `/opt/openssl/lib`, `build/lib`). `BUILD_WITH_INSTALL_RPATH ON` makes the build-tree binary link with `INSTALL_RPATH` instead of the auto set; `INSTALL_RPATH_USE_LINK_PATH` defaults OFF so nothing else is appended. libzstd is a system lib on the glibc-old image, not bundled.
