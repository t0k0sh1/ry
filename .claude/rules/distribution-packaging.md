...
- `ry self-update` is primarily a healthy-install forwarder to sibling `ry-self-update`; it must `execv` with inherited stdin/stdout/stderr rather than capture and replay output. Keep a built-in repair fallback for installs that upgraded from a release whose updater did not install the companion yet (for example v0.0.31 -> v0.0.32). Broken main-binary installs still recover through direct `ry-self-update` or `ry-rescue`.
