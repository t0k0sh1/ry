...
**Extracting a shared helper from near-duplicate emission blocks (the #2026 dedup) must preserve every SSA value name byte-for-byte — including the named-vs-unnamed asymmetry.** LLVM value names appear verbatim in textual IR and unnamed values are auto-numbered, so flipping one named value to unnamed (or vice versa) renumbers every downstream unnamed value and the diff is no longer empty even though the IR is semantically identical. Concrete trap from `emit_list_grow` (`crates/emit/src/support.rs`, shared by `ry_emit_collection_append` / `_insert`): the `cap > 4` compare is named `cap_gt4` in append but left **unnamed** in insert. A naive `prefix`-derived helper that names (or prefixes) both breaks bit-exactness. Pass the divergent name as an explicit param (`gt4_name = c"cap_gt4"` for append, `c""` for insert) and derive the rest from a `prefix` arg via `cname_pfx`. SSA-name-stable helpers like the libc emitters (`emit_malloc` takes the result `name`; `emit_memcpy` / `emit_memmove` / `emit_free` are unnamed at every site) and `emit_any_retain_guard` (names identical across wrap / unwrap / try_unwrap, only the pointer differs) extract cleanly.
...
In the `crates/emit/src/` emission modules (since #2025 the bodies live in per-op modules — `arc.rs` / `collection.rs` / `cow.rs` / `result.rs` / `bounds.rs` / `control_flow.rs` — not the former single `lib.rs`), when an boundary helper creates new `BasicBlock`s, the parent function MUST come from `ctx->builder->GetInsertBlock()->getParent()`, NOT from `ctx->function` (the cached pointer set via `ry_emit_ctx_set_function`). The cached pointer reflects `cg.fn_` at set-time, but ARC / lambda / thunk / destructor / iterator-next codegen retargets `builder_` to a nested function without immediately mirroring that change into `cg.fn_`. Calling an boundary helper from one of those contexts then creates new BBs in the outer function while the builder appends instructions (CondBr / Br targets) in the nested function — LLVM verify rejects with `Referring to an instruction in another function!` (#1968). Existing helpers that follow this rule: `ry_emit_arc_retain` / `ry_emit_arc_release` (post-fix); `ry_emit_collection_append` / `ry_emit_collection_insert` / `ry_emit_collection_remove_at` (#1971; `ry_emit_list_slice` creates no new BBs so the rule trivially holds); `ry_emit_cow_ensure_unique` (#1970 — invoked from CoW paths that may be reached via lambda / `@parallel for` / nested-collection destructor sites); `ry_emit_result_branch` (#1968); `ry_emit_bounds_check` (#1996 — the last helper aligned onto this pattern). All BB-creating boundary helpers now derive the parent function from the builder, not from `ctx->function`; use the builder-derived form unconditionally for new helpers too. The `ry_emit_ctx_set_function` boundary entry can be deprecated in a future Stage when no helper still reads `ctx->function`.
