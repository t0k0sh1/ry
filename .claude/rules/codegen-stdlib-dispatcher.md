...
The cost of bare `return nullptr;` is that sibling dispatchers that follow the "insert at the top" rule pollute `used_native_libraries_` with libraries that will be dlopen'd at JIT time but never called. Empirically (measured via the regression test below, #2299), a base64-only program currently loads `{base64, io}` — `dispatchIO` is the only dispatcher with an unconditional top-insert; `dispatchJson` / `dispatchJson5` have `isJson*Imported` sig-presence gates that suppress the insert when the package is not imported; `dispatchNet` / `dispatchHttp` / `dispatchPath` / `dispatchThread` / `dispatchRuntimeInternal` insert only via `emitTableDrivenNativeCall` and so do not fire on a non-matching call. The single spurious `io` load is wasteful but not a correctness bug, and resolves as `dispatchIO` (and any future unconditional-top-insert dispatcher) migrates to descriptor-driven dispatch.
...
- The `GetRequiredLibrariesOnlyIncludesCalledFunctions` regression test (`tests/test_codegen_directive.cpp`) asserts the exact transitional set via `EXPECT_EQ(libs, expected_libs)` where `expected_libs = {"base64", "io"}` (#2299). A new dispatcher silently joining the set, or an existing dispatcher's insert behavior changing, will fail the assertion and require an explicit `expected_libs` update. Collapse to `{"base64"}` when `dispatchIO` (and any future unconditional-top-insert dispatcher added meanwhile) migrate to descriptor-driven dispatch — that point is also the close criterion for #2299 and restores the original `libs.size() == 1` invariant.
