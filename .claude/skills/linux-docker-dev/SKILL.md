...
> `scan-build` and `all` isolate their analyzer-wrapped CMake configuration in `build-scan-docker/` (host) ↔ `build-scan/` (container), separate from `build-docker/`. This means a subsequent `./docker/run.sh default ...` works immediately without an intervening `rm -rf`. To discard the analyzer state and report, re-run with `--clean` (`run-scan-build.sh --clean`) rather than a manual `rm -rf` (AGENTS.md §"Total ban on Claude-initiated ad-hoc deletion").
...
`entrypoint.sh` adds three fail-fast guards on startup, all consulting `$RY_HOST_BUILD_DIR` (set by `run.sh`) so the recovery hint names the host-side directory to reset with `--clean`:
...
- **`--rebuild` invalidates host `build-*-docker/` caches when the image's toolchain layout changes** (e.g. ninja moved from `/usr/bin/ninja` to `/opt/ninja/bin/ninja` between image revisions). Symptom: `cmake: Running '/usr/bin/ninja' '--version' failed`. Remediation: re-run each affected preset with `--clean`, which removes that preset's host `build-*-docker/` before rebuilding — e.g. `./docker/run.sh --clean asan ry_tests` or the wrapper `run-asan.sh --clean` (static-analysis: `run-scan-build.sh --clean`). Prefer `--clean` over a manual `rm -rf build-*-docker/` (AGENTS.md §"Total ban on Claude-initiated ad-hoc deletion"). CMake reuses `CMAKE_MAKE_PROGRAM` from the cache; only `--fresh` or a `--clean` wipe forces re-discovery.
