...
The parallel spec runner also limits itself to four concurrent subprocesses
(`src/jit/test_runner.cpp`). At eight workers, #2172 reproduced the same
`collection_meta_propagation.test.ry` glibc SIGABRT within 3 Linux runs; the
same file passed 20/20 when run alone. Do not raise the cap without repeated
Linux default-build verification of this crash family.
